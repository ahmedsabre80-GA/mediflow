...
function authHeaders(json = true): Record<string, string> {
  const t = typeof window !== 'undefined' ? localStorage.getItem('warehouse-token') : null;
  return { ...(json ? { 'Content-Type': 'application/json' } : {}), ...(t ? { Authorization: `Bearer ${t}` } : {}) };
}

async function resolveWarehouseId(): Promise<string | null> {
  const r = await fetch(`${API}/warehouses/me`, { headers: authHeaders(false) });
  if (!r.ok) return null;
...
export default function UpgradeRequestPage() {
  ...
  useEffect(() => {
    (async () => {
      const id = await resolveWarehouseId();
      if (id) { setEntityId(id); await load(id); } else { setLoading(false); }
    })();
  }, []);
  ...
  return (
    <div className="space-y-6" dir="rtl">
      {message && <div className="bg-amber-50 border border-amber-200 text-amber-700 text-sm rounded-xl px-4 py-3">{message}</div>}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {plans.map(p => {
          ...
          return (
            <div key={p.id} className={`bg-white rounded-2xl border p-5 flex flex-col ${isCurrent ? 'border-amber-400 ring-1 ring-amber-200' : 'border-gray-200'}`}>
              <h3 className="font-bold text-gray-900 mb-1">{p.name_ar || p.name}</h3>
              {isCurrent && <span className="text-xs text-amber-600 font-medium mb-2">خطتك الحالية</span>}
              ...
              <button
                ...
                className="w-full bg-amber-500 hover:bg-amber-600 disabled:opacity-40 disabled:cursor-not-allowed text-white text-sm font-semibold py-2.5 rounded-xl transition-colors">
