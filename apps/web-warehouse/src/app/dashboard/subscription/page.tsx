...
// Warehouse localStorage only stores the owner's user id — the organization's
// entity_id is the warehouse row's own id, so it must be resolved via /me first.
async function resolveWarehouseId(): Promise<string | null> {
  const r = await fetch(`${API}/warehouses/me`, { headers: authHeaders() });
  if (!r.ok) return null;
...
export default function SubscriptionPage() {
  ...
  useEffect(() => {
    (async () => {
      const entityId = await resolveWarehouseId();
      if (!entityId) { setError('تعذر تحديد المستودع'); setLoading(false); return; }
      try {
        const r = await fetch(`${API}/organizations/${ENTITY_TYPE}/${entityId}/subscription`, { headers: authHeaders() });
                ...
                {q.is_metered && !unlimited && (
                  <>
                    <div className="w-full h-2 bg-gray-100 rounded-full overflow-hidden">
                      <div className={`h-full rounded-full ${pct >= 100 ? 'bg-red-500' : pct >= 80 ? 'bg-amber-500' : 'bg-amber-400'}`} style={{ width: `${pct}%` }} />
                    </div>
