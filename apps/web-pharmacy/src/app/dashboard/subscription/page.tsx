...
export default function SubscriptionPage() {
  ...
  useEffect(() => {
    const entityId = localStorage.getItem('pharmacy-id');
    if (!entityId) { setError('تعذر تحديد الصيدلية'); setLoading(false); return; }
    ...
    <div className="space-y-6" dir="rtl">
      <div className="bg-white rounded-2xl border border-gray-200 p-6 flex items-center justify-between flex-wrap gap-4">
        ...
        <Link href="/dashboard/subscription/upgrade"
          className="flex items-center gap-2 bg-sky-500 hover:bg-sky-600 text-white font-semibold px-4 py-2.5 rounded-xl text-sm transition-colors">
                ...
                {q.is_metered && !unlimited && (
                  <>
                    <div className="w-full h-2 bg-gray-100 rounded-full overflow-hidden">
                      <div className={`h-full rounded-full ${pct >= 100 ? 'bg-red-500' : pct >= 80 ? 'bg-amber-500' : 'bg-sky-500'}`} style={{ width: `${pct}%` }} />
                    </div>
