...
export default function UpgradeRequestPage() {
  ...
  useEffect(() => {
    const id = localStorage.getItem('pharmacy-id') || '';
    setEntityId(id);
  ...
  return (
    <div className="space-y-6" dir="rtl">
      {message && <div className="bg-sky-50 border border-sky-200 text-sky-700 text-sm rounded-xl px-4 py-3">{message}</div>}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {plans.map(p => {
          ...
          return (
            <div key={p.id} className={`bg-white rounded-2xl border p-5 flex flex-col ${isCurrent ? 'border-sky-400 ring-1 ring-sky-200' : 'border-gray-200'}`}>
              <h3 className="font-bold text-gray-900 mb-1">{p.name_ar || p.name}</h3>
              {isCurrent && <span className="text-xs text-sky-600 font-medium mb-2">خطتك الحالية</span>}
              ...
              <button
                ...
                className="w-full bg-sky-500 hover:bg-sky-600 disabled:opacity-40 disabled:cursor-not-allowed text-white text-sm font-semibold py-2.5 rounded-xl transition-colors">
